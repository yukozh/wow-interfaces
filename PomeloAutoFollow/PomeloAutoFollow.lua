...
function loadSettings()
    if settings == nil then
        ...
        settings.command1 = "11"
        settings.command2 = "22"
        ...
        settings.startFollowText = "已开始跟随"
        settings.stopFollowText = "已停止跟随"
    end
end
...
function CreatPanel()
    ...
    configurationPanel.name = "柚子自动跟随"
    ...
    introMessageHeader:SetText("柚子自动跟随 - PomeloAutoFollow")
	
    local VerTitle = configurationPanel:CreateFontString(nil, "ARTWORK", "NumberFontNormalSmall")
    VerTitle:SetPoint("TOPRIGHT", -10, -10)
    VerTitle:SetText("V0.9.0")
	
    local subText = configurationPanel:CreateFontString(nil, "ARTWORK", "GameFontHighlightSmall")
    subText:SetPoint('BOTTOMRIGHT', -10, 10)
    subText:SetText('作者: Yuko Zheng')
    ...
    local nameBox1 = CreateFrame("EditBox", "FollowUnit", configurationPanel)
    ...
    local nameBox2 = CreateFrame("EditBox","FollowUnit", configurationPanel)
    ...
    local nameBox3 = CreateFrame("EditBox","FollowUnit", configurationPanel)
    ...
    local chatMessageBox1 = CreateFrame("EditBox", "FollowUnit", configurationPanel)
    chatMessageBox1:SetMultiLine(false)
    chatMessageBox1:SetTextInsets(6, 10, 3, 5)
    chatMessageBox1:SetFontObject(ChatFontNormal)
    chatMessageBox1:SetWidth(300)
    chatMessageBox1:SetHeight(25)
    chatMessageBox1:SetPoint("TOPLEFT", 77, -290)
    chatMessageBox1:HighlightText()
    chatMessageBox1:SetMaxLetters(18)
    chatMessageBox1:SetAutoFocus(false)
    chatMessageBox1:SetBackdrop({
        _,
        edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
        _,
        _,
    ...
    local chatMessageBoxLabel1 = configurationPanel:CreateFontString(nil, "ARTWORK","GameFontWhiteSmall")
    chatMessageBoxLabel1:SetPoint("TOPLEFT", 385, -295)
    chatMessageBoxLabel1:SetText("开始跟随时发送的消息")

    local chatMessageBox2 = CreateFrame("EditBox", "FollowUnit", configurationPanel)
    chatMessageBox2:SetMultiLine(true)
    chatMessageBox2:SetTextInsets(6, 10, 3, 5)
    chatMessageBox2:SetFontObject(ChatFontNormal)
    chatMessageBox2:SetWidth(300)
    chatMessageBox2:SetHeight(25)
    chatMessageBox2:SetPoint("TOPLEFT", 77, -320)
    chatMessageBox2:HighlightText()
    chatMessageBox2:SetMaxLetters(18)
    chatMessageBox2:SetAutoFocus(false)
    chatMessageBox2:SetBackdrop({
        _,
        edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
        _,
        _,
    ...
    local chatMessageBoxLabel2 = configurationPanel:CreateFontString(nil, "ARTWORK","GameFontWhiteSmall")
    chatMessageBoxLabel2:SetPoint("TOPLEFT", 385, -325)
    chatMessageBoxLabel2:SetText("停止跟随时发送的消息")

    local introMessageHeader2 = configurationPanel:CreateFontString(nil, "ARTWORK","GameFontNormalLarge")
    introMessageHeader2:SetPoint("TOPLEFT", 10, -350)
    ...
    tradeCheckBox:SetPoint("TOPLEFT",10, -380)
    ...
    introMessageHeader3:SetPoint("TOPLEFT", 10, -425)
    ...
    colorCheckBox1:SetPoint("TOPLEFT",10, -450)
    ...
    colorCheckBox2:SetPoint("TOPLEFT",150, -450)
  ...
  configurationPanel.okay = function(self)
        ...
        settings.startFollowText = chatMessageBox1:GetText()
        settings.stopFollowText = chatMessageBox2:GetText()
	...
	configurationPanel.cancel = function(self)
        ...
        chatMessageBox1:SetText(settings.startFollowText)
        chatMessageBox2:SetText(settings.stopFollowText)
			...
			if event == "CHAT_MSG_PARTY_LEADER" then
				if arg1 == settings.command1 then
					...
					if settings.repoCheck == true then
						SendChatMessage(settings.startFollowText, "WHISPER", nil, arg5)
					end
					FollowUnit(arg5)
				end
				if arg1 == settings.command2 and settings.stopCheck == true then
					print("停止跟随")
					if settings.repoCheck == true then
						SendChatMessage(settings.stopFollowText, "WHISPER", nil, arg5)
					end
					FollowUnit("player")
				end
            end
...
SlashCmdList["TEST"] = function(msg)
    InterfaceOptionsFrame_OpenToCategory("柚子自动跟随")
end
