    ...
    do
        ...
        local templateMenu = {
            ...
            }, -- 0
            {
                ...
                notCheckable = true,
                func = function()
                    InterfaceOptionsFrame_OpenToCategory(L["Mirai Ledger"])
                    InterfaceOptionsFrame_OpenToCategory(L["Mirai Ledger"])
                end,
            },            
            { 
    ...
    do
        ...
        local beneficiaryUpdate = CreateCellUpdate(function(cellFrame, entry)
            if not cellFrame.textBox then
                ...
                cellFrame.textBox:SetScript("OnEscapePressed", cellFrame.textBox.ClearFocus)
                cellFrame.textBox:SetScript("OnEnterPressed", cellFrame.textBox.ClearFocus)
                cellFrame.textBox.mirailedgerbeneficiary = true
        ...
        local paidUpdate = CreateCellUpdate(function(cellFrame, entry)
            ...
            if entry["paidcache"] then
                cellFrame:SetScript("OnEnter", function()
                    tooltip:SetOwner(cellFrame, "ANCHOR_RIGHT")
                    tooltip:SetText(GetMoneyString(entry["paidcache"]))
                    tooltip:Show()
                end)
        ...
        local channelTypeMenu = {
            ...
            },
            {
                ...
                func = function ()
                    ...
                    for _, item in pairs(items or {}) do
                        if item["cost"] ~= item["paid"] then
                            local b = item["beneficiary"]
                            local c = item["cost"]
                            local p = item["paid"] or 0
                            local i = item["detail"]["item"] or ""
                            local d = item["detail"]["displayname"] or ""
                            if not GetItemInfoFromHyperlink(i) then
                                i = d
                            ...
                            debtor[b]["amount"] = debtor[b]["amount"] + c - p
                            table.insert(debtor[b]["items"], {i, c - p})
                        end
                    end
