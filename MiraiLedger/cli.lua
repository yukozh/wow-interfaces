...
hooksecurefunc("SetItemRef", function(link)
    if GUI.mainframe:IsShown() and IsShiftKeyDown() then
        ...
        elseif linkType == "player" then
            ...
            if edit and edit.mirailedgerbeneficiary then
...
RegEvent("ADDON_LOADED", function()
    ...
    icon:Register("MiraiLedger", ldb:NewDataObject("MiraiLedger", {
            ...
            OnTooltipShow = function(tooltip)
                tooltip:AddLine(L["Mirai Ledger"])
                tooltip:AddLine("https://mwow.org");
            end,
        }),  config)
end)
    ...
    while frame do
        if frame:GetName() == "AlertFrame" then
            ...
            local cleartoast = frame:AddSimpleAlertFrameSubSystem("MoneyWonAlertFrameTemplate", function(frame, text)
                ...
                frame.Label:SetText(L["Mirai Ledger"])
...
SlashCmdList["RAIDLEDGER"] = function(msg, editbox)
    ...
    if cmd == "" then
        ...
        -- Print("[".. L["/mirailedger"] .. " toggle] " .. L["toggle Auto recording on/off"])
